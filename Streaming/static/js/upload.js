...
const itemMarkup = (file, url, x, y) => {
  ...
  if (file.type.startsWith('image')) {
    ...
    `;
  } else if (file.type.startsWith('video')) { // Check if the file type is a video
  ...
  if (!nextButtonCreated) {
    // Create the next button if it's not created yet
    const nextButton = document.createElement("button");
    nextButton.classList.add("item-next");
    nextButton.textContent = "Next";
    list.appendChild(nextButton);
    
    // Set the flag to true indicating the next button is created
    nextButtonCreated = true;

    // Event listener for the next button
    nextButton.addEventListener("click", () => {
      // Handle next button click event
      // You can define a function to perform actions when next button is clicked
      const droppableElement = document.querySelector(".droppable");
      droppableElement.classList.add("form_field");
      // const formElement = document.querySelector(".form");
      // formElement.classList.add("form_data");
    });
  }
